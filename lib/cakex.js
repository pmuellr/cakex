...
var PROGRAM, _, addGlobalFn, cakex, daemon, defineModuleFunctions, fs, getNodeModulesScripts, getTime, invokeNodeModuleScript, log, main, path, pkg, sanitizeFunctionName, watch,
  slice = [].slice;
...
defineModuleFunctions = function(dir) {
  var fn, i, len, nodeModulesBin, sanityName, scriptName, scripts;
...
getNodeModulesScripts = function(dir) {
  var i, len, name, result, script, scripts;
  if (!test("-d", dir)) {
    return [];
  }
  result = {};
...
invokeNodeModuleScript = function(scriptPath, script) {
  ...
  return function() {
    ...
    commandArgs = arguments[0], execArgs = 2 <= arguments.length ? slice.call(arguments, 1) : [];
